...
function normalizeStreamSection(section) {
...
/**
 * SSE(section/done/photos) 또는 JSON(캐시·청크0) 응답을 통합 처리.
 * @param {object} params - query, signal, city, ...
 * @param {{ onSection?, onDone?, onPhotos? }} [handlers]
...
export async function searchStreaming(params, handlers = {}) {
  ...
  await readSseStream(res.body, {
    onSection: async (section) => {
      ...
      await handlers.onSection?.(section);
    },
