...
export function ensureReviewRefs(data) {
  ...
  // 스트리밍 section 이벤트는 sources보다 먼저 올 수 있음 — 리뷰를 비우지 않고 보존
  if (sourceIds.size === 0) {
    return data;
  }
  ...
  const syncReview = (r) => {
    ...
    if (ref == null) {
      const inferred = inferRefFromSources(r.text, sources);
      if (inferred != null && sourceIds.has(inferred)) {
        return { ...r, ref: inferred };
      }
      // ref 동기화 실패해도 본문은 유지 (배지 없이 노출 가능)
      return r;
    }
    return { ...r, ref };
  };

  const pickOpts = { trustAssigned: true };
