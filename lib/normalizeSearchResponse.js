...
import { ensureReviewRefs, extractRefFromReviewText } from "./ensureReviewRefs.js";
import { remapSourceRefs } from "./remapSourceRefs.js";
import { expandMergedPlaceDetail, sanitizePlaceDescription } from "./sanitizePlaceDescription.js";
...
function normalizeReview(r) {
  ...
  if (!text) return null;
  let ref = r.ref != null ? toInt(r.ref) : undefined;
  if (ref == null) ref = extractRefFromReviewText(text);
  return {
    text,
...
function normalizePlaceDetail(pd) {
  ...
  const reviews = Array.isArray(pd.reviews)
    ? pd.reviews.map(normalizeReview).filter(Boolean)
    : [];
  return {
    name: pd.name ?? "",
    description: sanitizePlaceDescription(pd.description ?? "", pd.name ?? ""),
    reviews,
...
function normalizePlaceDetailsList(list) {
  if (!Array.isArray(list)) return [];
  return list
    .flatMap((pd) => expandMergedPlaceDetail(pd))
...
function normalizeSection(s) {
  ...
  const places_detail = normalizePlaceDetailsList(s.places_detail);
...
function placeNamesLooseMatch(a, b) {
  const na = String(a ?? "").replace(/\*\*/g, "").trim();
...
function mergePlacesDetailLists(streamed, done) {
  if (!Array.isArray(done) || done.length === 0) return streamed ?? [];
  if (!Array.isArray(streamed) || streamed.length === 0) return done;

  return done.map((pd) => {
    const old = streamed.find((p) => placeNamesLooseMatch(p.name, pd.name));
    const reviews =
      Array.isArray(pd.reviews) && pd.reviews.length > 0
        ? pd.reviews
        : (old?.reviews ?? []);
    return {
      ...old,
      ...
      description: String(pd.description ?? "").trim()
        ...
        : (old?.description ?? ""),
      warnings:
        Array.isArray(pd.warnings) && pd.warnings.length > 0
          ? pd.warnings
          : (old?.warnings ?? []),
    };
  });
...
/** 스트리밍 중 수집한 content와 done payload의 places_detail/reviews 병합 */
export function mergeStreamSections(streamed, doneSections) {
  const stream = Array.isArray(streamed) ? streamed : [];
  const done = Array.isArray(doneSections) ? doneSections : [];
  if (done.length === 0) return stream;

  const maxLen = Math.max(stream.length, done.length);
  const out = [];
  for (let i = 0; i < maxLen; i++) {
    ...
    out.push({
      ...s,
      ...d,
      content: String(s.content ?? "").trim() ? s.content : (d.content ?? ""),
      title: String(s.title ?? "").trim() ? s.title : (d.title ?? ""),
      icon: s.icon ?? d.icon ?? "",
      places_detail: mergePlacesDetailLists(s.places_detail, d.places_detail),
      table: s.table ?? d.table ?? null,
    });
  }
  return out;
}
...
export function normalizeSearchResponse(raw) {
  ...
  return ensureReviewRefs(
    remapSourceRefs({
      ...
      search_id:
        raw.search_id != null && String(raw.search_id).trim()
          ? String(raw.search_id).trim()
          : null,
    })
  );
}
